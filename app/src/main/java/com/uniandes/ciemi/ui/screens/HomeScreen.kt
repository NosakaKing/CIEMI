...
fun HomeScreen(
    ...
    LaunchedEffect(selectedTabIndex, negocioSeleccionado) {
        negocioSeleccionado.let {
            when (selectedTabIndex) {
                0 -> {
                    viewModel.loadEarningProducts(context, it.id)
                    viewModel.loadEarningClients(context, it.id)
                    viewModel.loadCategory(context, it.id)
                }
                1 -> {
                    viewModel.loadStatus(context, it.id)
                    viewModel.loadClasification(context,it.id)
                    viewModel.loadMes(context, it.id)
                }
                2 -> {
                    viewModel.loadSalesMes(context, it.id)
        ...
        if (earningCategories.isNotEmpty()) {
            ...
            PieChart(
                ...
                onPieClick = { clickedPie ->
                    ...
                    println("${clickedPie.label} seleccionado")
                },
                selectedScale = 1.2f,
        ...
        items(status) { statusItem ->
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Text("Stock Minimo: ${statusItem.stockMin}")
                    Text("Stock Maximo: ${statusItem.stockMax}")
